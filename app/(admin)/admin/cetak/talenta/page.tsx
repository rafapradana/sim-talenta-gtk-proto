...
// Fake GTK Data
const fakeGtkData: GtkData[] = Array.from({ length: 50 }, (_, i) => ({
    id: `gtk-${i + 1}`,
    namaLengkap: [
    ...
    talentaCount: Math.floor(Math.random() * 5) + 1,
    foto: `https://api.dicebear.com/7.x/avataaars/svg?seed=gtk${i + 1}`,
}));

export default function CetakTalentaPage() {
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold">Cetak Talenta</h1>
                    <p className="text-sm text-muted-foreground">Pilih GTK untuk mencetak rekapitulasi talenta</p>
                </div>
                {selectedIds.size > 0 && (
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                <div className="flex gap-2 flex-1">
                    <div className="relative flex-1 max-w-sm">
                        ...
                        <Input
                            placeholder="Cari nama, NUPTK, atau sekolah..."
                            value={search}
            ...
            <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
                <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle>Cetak Rekapitulasi Talenta</DialogTitle>
                        <DialogDescription>
                            Isi detail cetakan dan konfirmasi GTK yang akan dicetak
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="space-y-4">
                        <div className="space-y-2">
                            ...
                            <Input
                                placeholder="Contoh: Rekapitulasi Talenta GTK SMAN 1 Malang"
                                value={printTitle}
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="max-h-48 overflow-y-auto rounded-md border p-2 space-y-2">
                                {selectedGtk.map((gtk) => (
                                    <div key={gtk.id} className="flex items-center gap-3 py-1">
                                        <Avatar className="h-6 w-6">
                                            <AvatarImage src={gtk.foto} alt={gtk.namaLengkap} />
                                            <AvatarFallback>{gtk.namaLengkap.charAt(0)}</AvatarFallback>
                                        </Avatar>
                                        <div className="flex-1 min-w-0">
                                            <p className="text-sm font-medium truncate">{gtk.namaLengkap}</p>
                                            <p className="text-xs text-muted-foreground">{gtk.sekolah}</p>
                                        </div>
                                        <Badge variant="outline" className="text-xs">{gtk.talentaCount} Talenta</Badge>
                                    </div>
                                ))}
                            </div>
                        </div>
